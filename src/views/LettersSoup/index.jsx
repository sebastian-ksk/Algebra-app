...
import { guardarPalabrasEncontradas } from '../../redux/slices/sopaDeLetras';
...
export const LettersSuop = () => {
  ...
  const almacenateClick = ({ id, letter }) => {
    ...
    if (isIds) return;
    setWordSucces([...WordSucces, { Ids: IdsCilick, word: objexExist.word }]);
    cleanWorsSelected();
  };
  ...
  const enddExercice = () => {
    ...
    dispatch(guardarPalabrasEncontradas({ num: numberWords, palabras: words }));
    navigate('/final');
          ...
          <ConLeterSoup>
            <ConponSoup>
              {lentdata.map((data, i) => {
                ...
                const isCLick = WordClick.Ids.some((item) => item === i);
            ...
            <ContainesWords>
              ...
              {dataWords.map((daW) => {
                const isWord = WordSucces.some((data) => data.word === daW);
                return <WordsSty state={isWord}>- {daW}</WordsSty>;
              })}
              <br />
              <br />
              <Button
